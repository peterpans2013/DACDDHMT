...
class GameView {
    ...
    for (let i = 0; i < 5; i++) {
      if (i === 1 || i === 3) {
        ...
        lineGeometry.vertices.push(new THREE.Vector3(
            this.xPos[i], this.yStartPoint + 12, this.zStartPoint));
        lineGeometry.vertices.push(new THREE.Vector3(
            this.xPos[i], this.yEndPoint + 12, this.zEndPoint));
  ...
  addMovingNotes(noteInterval) {
    ...
    songNotes.forEach((songNote, idx) => {
      ...
      // // CREATE HOLDS
      // if (songNote.hold) {
      //   let cylinderMaterial = this.note.materials[6];
      //   let cylinderGeometry = new THREE.BoxGeometry( 50, songNote.hold * this.note.vel * 10, 20 )
